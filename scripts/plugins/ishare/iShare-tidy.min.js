...
(function(root, factory) {
        ...
        return {
            ...
            render: function() {
                ...
                var _containerHTML = '<div style="text-align: center;background-color: ' + _bgcolor + ';box-shadow: 1px 1px 4px #888888;padding: 8px 8px 4px;border-radius: 4px;' + _radius + '">',
                    ...
                    _upArrowHTML = '<div class="ishare-qrcode-arrow" style="' + _upFlag + 'position: relative;height: 0;width: 0;border-style: solid;border-width: 12px;border-color: transparent;border-bottom-color: ' + _bgcolor + ';border-top: none;"></div>',
                ...
                this.wxbox.setAttribute('class', 'ishare-qrcode-box');
            ...
            setLocation: function(flag) {
                ...
                if (rect.right + this.wxbox.offsetWidth < sw) {
                    ...
                    _boxStyle = _boxStyle + 'right: ' + (sw - rect.right + 10) + 'px;';
                    if (this.upDownFlag === 'down') {
                        _boxStyle = _boxStyle + 'top: ' + (_eTop - _boxH) + 'px;';
                    } else {
                        _boxStyle = _boxStyle + 'top: ' + (_eTop + _eH) + 'px;';
                    ...
                    for (var i = 0; i < this.wxbox.childNodes.length; i++) {
                        if (this.wxbox.childNodes[i].className == 'ishare-qrcode-arrow') {
                            var arrow = this.wxbox.childNodes[i];
                            arrow.style.cssText += 'left: 100px';
                        }
                    }
                }
    ...
    iShare.prototype = (function() {
        ...
        function _updateUrl() {
            ...
            var _children = this.container.getElementsByTagName('a'),
                _tempURL;
            for (var i = 0, item; item = _children[i++];) {
                if (item.nodeType === 1 && item.className.indexOf('iShare_') > -1) {
                    ...
                    if ((item.className).indexOf('iShare_wechat') > -1 && !(Util.isWeixinBrowser())) {
                        ...
                        this.wx.push(new WX(item, this.settings.url, this.settings.WXoptions));
                    } else {
                        item.className += ' external';
        ...
        return {
            ...
            init: function() {
                ...
                if (this.wx.length) {
                    for (var i = 0; i < this.wx.length; i++) {
                        var wx = this.wx[i];
                        this.bindEvent(wx);
                        wx.init();
                        // 加载qrcode库
                        if (typeof QRCode === undefined) {
                            Util.loadjs('qrcode.min.js', wx.startQR());
                        } else {
                            wx.startQR()();
                        }
                    }
