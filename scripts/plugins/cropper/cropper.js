...
var render$1 = {
  ...
  initContainer: function initContainer() {
    ...
    $cropper.css(self.container = {
      width: Math.max($this.width(), Number(options.minContainerWidth) || 200),
      height: Math.max($this.height(), Number(options.minContainerHeight) || 100)
    });
...
var events = {
  bind: function bind() {
    ...
    $(document).on(EVENT_MOUSE_MOVE, self.onCropMove = proxy(self.cropMove, this));
    $(document).on(EVENT_MOUSE_UP, self.onCropEnd = proxy(self.cropEnd, this));
