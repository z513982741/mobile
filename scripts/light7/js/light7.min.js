...
;(function ($) {
	...
	*/
    ...
    default:
        ...
        if ($(target).hasClass('needsclick') || $(target).parents('.needsclick').length) {
            return true;
        }
	};
...
+function ($) {
  ...
  $(document).on("click", ".tab-item", function(e) {
    ...
    if(!$target.hasClass("tab-link")) {
        ...
        if ($target.parent().find(".active").length) {
            var $cur = $target.parent().find(".active");
            prevActiveIndex = $target.parent().find('.tab-item').index($cur);
        }
        $target.parent().find(".active").removeClass("active");
        $target.addClass("active");
        $target.attr('prev-active-index', prevActiveIndex);
    }
  });
  ...
  "use strict";
    ...
    $.modal = function (params) {
        ...
        if (defaults.modalTemplate) {
            ...
            var customClass = params.customClass ? params.customClass : '';
            var titleHTML = params.title ? '<div class="modal-title">' + params.title + '</div>' : '';
            var textHTML = params.text ? '<div class="modal-text">' + params.text + '</div>' : '';
            var afterTextHTML = params.afterText ? params.afterText : '';
            var noButtons = !params.buttons || params.buttons.length === 0 ? 'modal-no-buttons' : '';
            var verticalButtons = params.verticalButtons ? 'modal-buttons-vertical' : '';
            modalHTML = '<div class="modal ' + customClass + ' ' + noButtons + '"><div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div><div class="modal-buttons ' + verticalButtons + '">' + buttonsHTML + '</div></div>';
        }
    ...
    function handleClicks(e) {
        ...
        if (!$page.length || !$page.hasClass('page-inited')) {
...
+function ($) {
  ...
  Router.prototype.preroute = function(url) {
    return true;
  }

  Router.prototype.prepareUrl = function(url) {
    return url;
  }
  ...
  Router.prototype.loadPage = function(url, noAnimation, replace, reload, leftToRight) {
    ...
    if (!this.preroute(url)) {
        return false;
    }

    url = this.prepareUrl(url);

    this.referer =  $(location).prop('href');
    ...
    this.getPage(url, function(page, extra, extraIncludes) {
	  ...
	  if (page.find('a.back').length) {
        page.find('a.back').prop('href', this.referer);
      }
	  ...
	  $.when($(document).triggerHandler('pageInsert', [page, extraIncludes, out])).then(function() {
		  if (out.length) {
			  $.each(out, function(i, $el) {
				  extra = extra.add($el);
			  });
		  }
		  
		  if (extra.length) self.extras[page[0].id] = extra.appendTo(document.body);
		  self.animatePages(curPage, newPage, leftToRight, noAnimation);
	  });
    });
  }
  ...
  Router.prototype.back = function(url) {
    ...
    if(stack.length) {
      ...
      if ($('iframe').length) {
        this._back();
        var id = this.genStateID();
        this.replaceState(stack[stack.length - 1].url, id);
      } else {
        history.back();
      }
      */
  ...
  Router.prototype._back = function() {
    var currentPage = this.getCurrentPage();
    var result = $(document).triggerHandler('pageBeforeBack', [currentPage[0].id, currentPage]);
    if (false === result) {
        return;
    }
    ...
    if(!h) {
        var state = history.state;
        if (state) {
            $.router.loadPage(state.url, undefined, true, undefined, true);
        }
        return;
    }
    ...
    $(document).trigger('pageAfterBack', [h.pageid, newPage]);
  }
  ...
  Router.prototype.getPage = function(url, callback) {
    ...
    this.xhr = $.ajax({
      ...
      success: $.proxy(function(data, s, xhr) {
        if (typeof data === 'object' && data.data && data.data.redirecturl) {
            if (0 === data.data.redirecturl.indexOf('http')) {
                var getDomain = function(hostname) {
                    ...
                    if (parts.length == 3 && $.inArray(parts[1], ['com', 'net', 'org'])) {
                        return hostname;
                    }
                ...
                var hostnamex = $(location).prop('hostname');
                var hostname = $('<a>').prop('href', data.data.redirecturl).prop('hostname');
                var domainx = getDomain(hostnamex);
                var domain  = getDomain(hostname);
                if (domainx != domain) {
                   $(location).prop('href', data.data.redirecturl);
                   return;
                }
            }

            //self.replacePage(data.data.redirecturl);
            self.loadPage(data.data.redirecturl);
            return;
        }
        var html = this.parseXHR(xhr);
        var $page = html[0];
        var $extra = html[1];
		var $extraIncludes = html[2];
        if(!$page[0].id) $page[0].id = this.genRandomID();
        $page.data("page-remote", 1);
        callback.apply(this, [$page, $extra, $extraIncludes]);
      }, this),
      error: function() {
        console.log('page load error url: ' + url);
  ...
  Router.prototype.parseXHR = function(xhr) {
	...
	var $extraIncludes = tmp.contents().filter(function() {
		return this.nodeType === 8 && this.nodeValue.indexOf('include ') != -1;
	});
    ...
    return [$page, $extra, $extraIncludes];
  }
  ...
  $(function() {
    ...
    $(document).on("click", "a", function(e) {
      ...
      if ($target.hasClass('tab-item') && $target.parent().hasClass('bar-tab')) {
        var index = $target.parent().find('.tab-item').index($target);
        var prevActiveIndex = $target.attr('prev-active-index');
      ...
      router.loadPage(url, $target.hasClass("no-transition") ? true : undefined, $target.hasClass("replace") ? true : undefined, undefined, leftToRight);  //undefined is different to false
    })
  });
}($);
// jshint ignore: end
...
+function ($) {
    "use strict";
    var ScrollFix = function(fixContent, _options) {
        var $fixContent = this.$fixContent = $(fixContent);
		var $scroller   = this.$scroller = this.$fixContent.parents('.content');
		if (!$fixContent.length) {
			return;
		}
		
        //var shadow = $fixContent.clone();
        var fixedTop = $fixContent[0].getBoundingClientRect().top;
    ...
    ScrollFix.prototype = {
        ...
        _bindEvents: function() {
            this.$scroller.on('scroll', this._scrollHandler.bind(this));
        },
        
        // 滚动核心代码
        _scrollHandler: function(ev) {
            var $scroller = $(ev.target); //被滚动的区域
            var offset = this.options.offset;
            var fixedTop = this.options.fixedTop;
            var scrollTop = $scroller.scrollTop();
            var isFixed = scrollTop >= fixedTop - offset;
			var scrollto = $scroller.attr('scrollto');
			
			if (scrollto) {
				if (scrollTop >= parseInt(scrollto, 10)) {
					$scroller.removeAttr('scrollto');
					this.update();
				}
				return;
			}
		...
		update: function() {
			var fixedTop = this.$fixContent[0].getBoundingClientRect().top;
			console.log('scroller top: ' + this.$scroller.scrollTop());
			this.options.fixedTop = fixedTop;
			return this;
		},
		
		fixed: function() {
            var $fixContent = this.$fixContent; //被固定的区域
			if ($fixContent.hasClass('scrollfixed')) {
				return;
			}
			...
			//shadow.insertAfter($fixContent);
			
			if (!isFixed) {
				this.$scroller.attr('scrollto', fixedTop - offset);
				this.$scroller.scrollTop(fixedTop - offset);
			}
            $fixContent.addClass('scrollfixed').css('top', offset);
		},
		
		disFixed: function(toTop) {
            var $fixContent = this.$fixContent; //被固定的区域
            //var shadow = this.options.shadow;
            var offset = this.options.offset;
			//shadow.remove();
			...
			if (true === toTop) {
				...
				this.$scroller.attr('scrollto', '0');
			}

            $fixContent.removeClass('scrollfixed').css('top', 0);
		}
    };

    //ScrollFix PLUGIN DEFINITION
    // =======================
    ...
    function Plugin(option) {
        ...
        this.each(function() {
            ...
            var data = $this.data('scrollfixed');
            if (!data) {
                //获取data-api的
                $this.data('scrollfixed', (data = new ScrollFix(this, options)));
            }
        });
